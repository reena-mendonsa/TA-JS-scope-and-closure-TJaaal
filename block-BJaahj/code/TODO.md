...
```js
function schedule(callbacks,delays) {
   if(callbacks.length != delays.length){
     alert('Invalid Input');
   }
   else{
     callbacks.forEach((fn,index)=>{
     setTimeout(fn,delays[index]*1000) ;  
     });
